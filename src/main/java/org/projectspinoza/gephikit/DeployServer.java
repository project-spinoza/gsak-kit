...
public class DeployServer extends AbstractVerticle {
	...
	private void registerHandlers() {
		...
		router.route("/gephi").blockingHandler(routingContext -> {
			...
			try {
	            ...
	            calculatePageRank(true, basicGraph.getGraphModel(), getAttributeModel());
	...
	// gsakkit route to generate graph with the required filters and layouts
		router.route("/gsakkit").blockingHandler(routingContext -> {
			...
			String graphJson = "";
			try {
			    System.out.println(routingContext.request().getParam("basicSettings"));
				if (routingContext.request().getParam("basicSettings") != null) {
	...
	public void applyLayout(Map<String, Object> settings, GraphModel graphModel) throws Exception {
		...
		layoutManager = new LayoutManager(configurationManager.getConfiguration()
	...
	public void applyFilters(Map<String, Object> settings, Graph graph){
		...
		if(settings.containsKey("pageRankThreashhold")){
			filterImplementation.applyPageRank(graph, getAttributeModel(), graphFilter,Double.parseDouble( settings.get("pageRankThreashhold").toString()), "pageranks");
		}
		if(settings.containsKey("nodeCentrailityThreashhold")){
			filterImplementation.applyNodeCentralityThreashhold(graph, getAttributeModel(), graphFilter,  Double.parseDouble(settings.get("nodeCentrailityThreashhold").toString()), "Betweenness Centrality");
		}
		if(settings.containsKey("neighborRangeThreashhold")){
			filterImplementation.applyNeighborcountThreshHold(graph, getAttributeModel(), graphFilter, Double.parseDouble(settings.get("neighborRangeThreashhold").toString()), "NeighborCount");
		}
	...
	public void calculatePageRank(Boolean IS_DIRECTED, GraphModel graphModel, AttributeModel attributeModel){
	...
	public AttributeModel getAttributeModel(){
	    return attributeModel = Lookup.getDefault().lookup(AttributeController.class).getModel();
	}
